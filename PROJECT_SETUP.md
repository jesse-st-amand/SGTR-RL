...
*True Positive* (model's text → claims authorship):
```
...
*True Negative* (other model's text → doesn't claim authorship):
```
...
*False Positive* (other model's text → incorrectly claims authorship):
```
...
*False Negative* (model's text → incorrectly rejects authorship):
```
...
### Initial Experiment: Train IND → Test PW
...
**Success Criteria**:
- ✅ IND accuracy improves on WikiSum (training works)
- ✅ PW accuracy shows uplift on WikiSum (cross-OP transfer)
- ✅ MMLU performance maintained (no cooking)
- ✅ Inspect benchmarks stable (model quality preserved)
- 🎯 Bonus: IND improves on other domains (cross-domain transfer)
...
├── .gitignore                     # Python-standard gitignore patterns
├── .gitmodules                    # Git submodule configuration
├── .python-version                # Python 3.12 (matches self-rec-framework)
├── pyproject.toml                 # Package configuration and dependencies
├── README.md                      # Project documentation and setup instructions
├── PROJECT_SETUP.md               # This file - detailed project documentation
│
├── sgtr_rl/                       # Main package directory
│   ├── __init__.py               # Package initialization (version 0.1.0)
│   ├── data_processing/          # Data preparation for DPO training
│   │   ├── __init__.py
│   │   ├── cached_loader.py      # Load cached generations from self-rec-framework
│   │   ├── triple_generator.py   # Create (prompt, chosen, rejected) triples
│   │   └── mmlu_loader.py        # Load MMLU regularization data
│   ├── training/                 # RL training implementation
│   │   ├── __init__.py
│   │   ├── dpo_trainer.py        # DPO training logic with Tinker
│   │   ├── config.py             # Training configuration dataclasses
│   │   └── utils.py              # Training utilities
│   ├── evaluation/               # Evaluation integration
│   │   ├── __init__.py
│   │   ├── evaluator.py          # Evaluate checkpoints on SGTR tasks
│   │   ├── mmlu_eval.py          # MMLU evaluation
│   │   └── inspect_eval.py       # Inspect benchmark evaluation
│   ├── models/                   # Model wrappers and utilities
│   │   ├── __init__.py
│   │   ├── model_loader.py       # Load models from Together.ai
...
│   ├── 02_RL_PW_train_WikiSum/  # Train PW on WikiSum (future)
│   │   └── ...
│   ├── _scripts/                # Shared experiment runner scripts
...
✅ Git submodules configured (self-rec-framework, tinker-cookbook, inspect_ai)
✅ Python package structure created (`sgtr_rl/`)
✅ `pyproject.toml` configured with dependencies
✅ Documentation complete (README.md, PROJECT_SETUP.md)
...
7. **Costs**: Monitor training costs on Tinker dashboard. Budget ~£10-50 per training run.
...
**Expected Breakdown**:
- Training experiments: £500-1000 (10-20 runs at £50 each)
- Evaluation runs: £100-200 (50-100 evals at £2-5 each)
- Development/testing: £100-200
- Buffer for iterations: £2700-3300
