...
def evaluate_val(val_prompts, sampling_client, renderer, eval_params) -> dict:
    ...
    Args:
        val_prompts: List of dicts with 'prompt', 'target', and 'metadata'.
    ...
    Returns:
        ...
        and per-sample predictions (uuid, prediction, target, correct, logprob).
    """
    ...
    for future, item in zip(futures, val_prompts):
        ...
        target = item["target"]
        uuid = item.get("metadata", {}).get("uuid", "")
...
def run_val_eval(
    ...
    Consolidates the common val eval pattern used by both SFT and GRPO trainers.
